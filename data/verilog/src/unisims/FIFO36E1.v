...
"7SERIES" : begin
   ...
   always @(posedge RDCLK) begin
    ...
    EMPTY = 1'b1;
      ...
      if ((((rdcount_out + ae_empty) >= wr_addr) && (rdcount_flag == wr_flag)) || 
          (((rdcount_out + ae_empty) >= (wr_addr + addr_limit) && (rdcount_flag != wr_flag)))) begin
    ALMOSTEMPTY = 1'b1;
      end
...
endgenerate
  ...
  if (sync == 1)
    ...
    always @(wr_addr or posedge RST or posedge GSR) begin 
      if(RST || GSR) begin
        WRCOUNT = 13'd0;
      end
      else begin
    ...
    // matching HW behavior to pull up the unused output bits
    always @(rdcount_out or posedge RST or posedge GSR) begin 
      if(RST || GSR) 
        RDCOUNT = 13'd0;
      else begin
        if (FIFO_SIZE == 18)
